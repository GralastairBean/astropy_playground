...
def main() -> None:
    ...
    input_csv = data_dir / "gaia_10000_closest_stars.csv"
    output_plot = data_dir / "gaia_10000_hr_diagram.png"
    ...
    ax.set_title("Gaia 10,000 Closest Stars HR Diagram")
    ax.invert_yaxis()
    ax.grid(True, linestyle="--", alpha=0.3)

    plt.tight_layout()
    log(f"Saving HR diagram to {output_plot}...", started_at)
    plt.savefig(output_plot, dpi=300)
    plt.show()

    log(f"Saved HR diagram to: {output_plot}", started_at)
